...
- Moved `dev` dependencies from PEP 735 `[dependency-groups]` to `[project.optional-dependencies]` so that `pip install robotsix-llmio[dev]` works. Removed `[tool.uv] default-groups = ["dev"]` (no longer needed).
...
- **Removed deprecated `openrouter_deepseek` shim**: the backward-compatibility
  ...
  the canonical `robotsix_llmio.openrouter` path.
