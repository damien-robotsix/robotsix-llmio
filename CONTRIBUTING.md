...
- CI **must** pass — the test suite on Python 3.14, ruff, mypy,
  ...
  the PR diff.
- **Use a conventional-commit PR title** — the changelog is generated by
  release-please from commit types, so write a title prefixed with `feat:`,
  `fix:`, `break:`, or `chore:`. No hand-edited `CHANGELOG.md` or
  `changelog.d/` newsfragment is expected or required. A `chore:` title
  (changes with nothing to release) needs no special label.
...
1. On every push to `main`, release-please opens (or updates) a release PR that
   accumulates the version bump and the auto-generated `CHANGELOG.md` from the
   merged conventional-commit titles.
2. Merging that release PR cuts the release: it bumps the version, updates
   `CHANGELOG.md`, and creates the `v0.X.Y` tag.
3. As a fallback, the scheduled weekly `auto-release.yml` workflow triggers the
   same release-please flow if a release PR is pending.

`CHANGELOG.md` is never edited by hand — release-please generates it from the
conventional-commit types on `main`
